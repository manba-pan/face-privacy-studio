...
from PySide6.QtGui import QColor, QFont, QImage, QPainter, QPen, QShortcut, QKeySequence, QDesktopServices
...
class Viewer(QWidget):
    ...
    def __init__(self):
        ...
        self.drag=None;self.selection=None;self.rotation=0
...
class Timeline(QWidget):
    ...
    def paintEvent(self,event):
        ...
        for i in range(7):
            ...
            p.setPen(QColor('#8692a6'));p.drawText(QRectF(x-27,4,70,22),Qt.AlignmentFlag.AlignCenter,f'{c.info.duration*i/6:.1f}s')
        ...
        if c.wave:
            ...
            p.setPen(QColor('#689083'));p.drawText(QRectF(self.x(0)+10,72,220,34),Qt.AlignmentFlag.AlignVCenter,'无音轨' if not c.media['audio_codec'] else '音频波形将在分析后显示')
        if c.analysis:
...
class Studio(QMainWindow):
    def __init__(self):
        ...
        self.player.setAudioOutput(self.audio);self.sink=QVideoSink(self);self.player.setVideoSink(self.sink)
        ...
        self.player.errorOccurred.connect(lambda *_:self.status.setText('系统预览解码不可用，可逐帧检查并正常导出。'))
    ...
    def build_ui(self):
        ...
        self.export_button=button('导出视频  ↗',self.export_current,True);top.addWidget(self.export_button);layout.addLayout(top)
        ...
        self.analyze_button=button('重新分析当前素材',self.analyze_current);ll.addWidget(self.analyze_button)
        ...
        self.viewer=Viewer();self.viewer.rectangle.connect(self.draw_rectangle);cl.addWidget(self.viewer,1)
        ...
        self.timeline=Timeline();self.timeline.seek.connect(self.seek);bl.addWidget(self.timeline);self.vertical.addWidget(bottom)
        self.vertical.setSizes([640,190]);self.vertical.setStretchFactor(0,1)
        statusrow=QHBoxLayout();self.status=label('就绪 · 导入后自动分析；Space 播放，← → 逐帧，I / O 截取','muted');statusrow.addWidget(self.status,1)
    ...
    def build_mask_tab(self):
        box=self.page('遮挡');self.auto=QCheckBox('自动遮挡检测到的人脸');self.auto.setChecked(True);box.addWidget(self.auto)
    ...
    def update_actions(self):
        ...
        self.export_button.setEnabled(bool(c and c.analysis and not self.busy))
        self.snapshot_button.setEnabled(bool(c and c.analysis and not self.busy))
        self.extract_button.setEnabled(bool(c and c.media['audio_codec'] and not self.busy))
        self.play_button.setEnabled(bool(c and c.analysis));self.add_box.setEnabled(bool(c and c.analysis));self.key_box.setEnabled(bool(c and c.analysis))
    def start_job(self,description,worker,callback):
        if self.busy:return
        self.player.pause();self.busy=True;self.cancel=threading.Event();self.callback=callback;self.started=time.monotonic();self.job_description=description
        ...
        def run():
            ...
            except core.Cancelled:self.bridge.failed.emit('处理已取消，源文件未改动。')
    ...
    def finish_job(self):
        self.busy=False;self.progress.hide();self.cancel_button.hide();self.update_actions()
    def on_done(self,result):
        callback=self.callback;self.finish_job();callback(result)
    def on_failed(self,text):
        self.finish_job();self.status.setText(text)
        if '取消' not in text:self.message(text)
    def import_dialog(self):
        if self.busy:return
        paths,_=QFileDialog.getOpenFileNames(self,'导入视频','',VIDEO_FILTER)
        if paths:self.import_paths(paths)
    def import_paths(self,paths,analyze=True):
        if self.busy:return
        first=len(self.clips);errors=[]
        for path in paths:
            try:
                absolute=str(Path(path).resolve())
                if any(c.info.path==absolute for c in self.clips):continue
                info=core.probe(absolute);c=Clip(info,exporter.details(absolute));self.clips.append(c)
                self.bin.addItem(self.clip_text(c))
            except Exception as error:errors.append(f'{Path(path).name}: {error}')
        self.clip_count.setText(f'{len(self.clips)} 个')
        if len(self.clips)>first:
            self.bin.setCurrentRow(first)
            if analyze:self.analyze_current()
        if errors:self.message('\n'.join(errors))
    def clip_text(self,c):
        status='已分析' if c.analysis else '待分析'
    ...
    def select_clip(self,index):
        self.pending_seek=None;self.pending_play=False
        self.player.stop();self.player.setSource(QUrl());self.current=index;self.index=0;self.raw=None;self.stop_drawing()
        c=self.clip
        if not c:self.viewer.image=None;self.viewer.update();self.timeline.clip=None;self.timeline.update();self.update_actions();return
        self.loading=True
        try:
            ...
            self.meta.setText(f'{c.info.width} × {c.info.height}   /   {c.info.fps:g} fps   /   {c.media["audio_codec"] or "无音轨"}     ·     轻量预览缓存')
            self.box_list.clear()
            for i,box in enumerate(c.manual):self.box_list.addItem(self.box_text(i,box))
            self.review_list.clear()
            if c.analysis:
                self.player.setSource(QUrl.fromLocalFile(c.analysis.preview_path))
                for r in c.analysis.review:
                ...
                self.raw=core.read_preview(c.analysis,0)
            else:self.raw=core.read_frame(c.info,0,core.fit_size(c.info.width,c.info.height))
            self.timeline.clip=c
        finally:self.loading=False
        self.controls_changed();self.update_actions()
    def analyze_clip(self,c,progress):
        analysis=core.analyze(c.info,self.cancel,progress)
        try:
            exporter.preview_with_audio(analysis,self.cancel)
            wave=exporter.waveform(analysis.preview_path)
            core.check_cancel(self.cancel)
            return analysis,wave
        except Exception:analysis.close();raise
    def analyze_current(self):
        c=self.clip
        if not c or self.busy:return
        old_index=self.index if c.analysis else None
        self.player.stop();self.player.setSource(QUrl())
        def worker():return self.analyze_clip(c,lambda value,*_:self.bridge.progress.emit(value*.94,'分析人脸与预览'))
        def done(result):
            if c.analysis:c.analysis.close()
            c.analysis,c.wave=result
            c.options.end_frame=min(c.options.end_frame,c.info.frames) if c.options.end_frame>=0 else -1
            if c.options.start_frame>=c.info.frames:c.options.start_frame=0
            self.bin.item(self.current).setText(self.clip_text(c));self.select_clip(self.current)
            if old_index is not None:self.seek(old_index)
            else:
                runs=[];start=None
                for i,faces in enumerate(c.analysis.faces+[[]]):
                    if faces and start is None:start=i
                    elif not faces and start is not None:runs.append((i-start,start,i));start=None
                if runs:
                    _,start,end=max(runs);self.seek((start+end)//2)
            self.status.setText(f'分析完成 · {sum(bool(f) for f in c.analysis.faces)} / {c.info.frames} 帧检测到人脸 · 请回看橙色标记片段')
        self.start_job('分析人脸与预览',worker,done)
    ...
    def refresh_frame(self,*_):
        c=self.clip
        if not c or self.raw is None:return
        faces=c.analysis.faces[self.index] if c.analysis and self.index<len(c.analysis.faces) else []
        self.viewer.rotation=c.options.rotation
        if self.compare.isChecked() or not c.analysis:rgb=exporter.rotate(self.raw,c.options.rotation)
        else:rgb=exporter.mask_render(self.raw,faces,c.settings,c.manual,self.index,c.options)
        self.viewer.set_image(rgb)
        row=self.box_list.currentRow();self.viewer.selection=core.manual_rect(c.manual[row],self.index) if 0<=row<len(c.manual) and c.manual[row]['start']<=self.index<=c.manual[row]['end'] else None
        self.detection.setText(('原画对比' if self.compare.isChecked() else '打码预览')+f'   ·   第 {self.index+1} 帧   ·   '+(f'检测到 {len(faces)} 张脸' if c.analysis else '尚未分析'))
        self.clock_label.setText(f'{timecode(self.index/c.info.fps)}  /  {timecode(c.info.duration)}')
        self.timeline.index=self.index;self.timeline.update()
    def video_frame(self,frame):
        c=self.clip
        if not c or not c.analysis or not frame.isValid() or self.player.playbackState()!=QMediaPlayer.PlaybackState.PlayingState:return
        now=time.monotonic()
        if now-self.last_paint<1/32:return
        self.last_paint=now
        ...
        im=frame.toImage().convertToFormat(QImage.Format.Format_RGB888)
        if im.isNull():return
        self.raw=np.frombuffer(im.constBits(),np.uint8,count=im.sizeInBytes()).reshape(im.height(),im.bytesPerLine())[:,:im.width()*3].reshape(im.height(),im.width(),3).copy()
        self.index=index;self.refresh_frame()
    ...
    def seek(self,index):
        c=self.clip
        if not c or not c.analysis:return
        self.pending_play=False;self.player.pause();self.index=max(0,min(c.info.frames-1,index))
        ...
        self.raw=core.read_preview(c.analysis,self.index);self.refresh_frame()
    def toggle_play(self):
        c=self.clip
        if not c or not c.analysis:return
        if self.player.playbackState()==QMediaPlayer.PlaybackState.PlayingState:self.player.pause();return
        self.stop_drawing();end=c.options.end_frame if c.options.end_frame>=0 else c.info.frames
        if self.index>=end-1 or self.index<c.options.start_frame:self.seek(c.options.start_frame)
        position=round(self.index/c.info.fps*1000)
        if self.player.isSeekable():
            self.pending_seek=None;self.player.setPosition(position);self.player.play()
        else:
            self.pending_seek=position;self.pending_play=True;self.player.play()
    def mark_in(self):
        c=self.clip
        if not c:return
        end=c.options.end_frame if c.options.end_frame>=0 else c.info.frames
    ...
    def new_box(self):
        if not self.clip or not self.clip.analysis:return
        self.player.pause();self.draw_target=-1;self.viewer.drawing=True;self.viewer.setCursor(Qt.CursorShape.CrossCursor);self.viewer.update()
    ...
    def export_current(self):
        c=self.clip
        if not c or self.busy:return
        if not c.analysis:self.analyze_current();return
        ...
        def worker():return exporter.export(c.analysis,path,settings,options,manual,self.cancel,lambda v,*_:self.bridge.progress.emit(v,'高质量导出'),c.media)
        self.start_job('高质量导出',worker,self.export_done)
    def export_done(self,path):
        self.last_output=str(path);self.folder_button.setEnabled(True);self.status.setText(f'导出完成 · {path}')
    def export_batch(self):
        ...
        def worker():
            ...
            for number,(c,settings,options,manual) in enumerate(jobs):
                ...
                if not c.analysis:c.analysis,c.wave=self.analyze_clip(c,lambda v,*_:progress(v*.5))
                ...
                outputs.append(exporter.export(c.analysis,path,settings,options,manual,self.cancel,lambda v,*_:progress(.5+v*.5),c.media))
            return outputs
        def done(outputs):
    ...
    def snapshot(self):
        c=self.clip
        if not c or not c.analysis or self.busy:return
        ...
        try:
            ...
            rgb=exporter.mask_render(raw,c.analysis.faces[self.index],c.settings,c.manual,self.index,c.options)
    ...
    def project_data(self):
        return {'format':'face-privacy-studio','version':1,'clips':[{'path':c.info.path,'fingerprint':list(core.fingerprint(c.info.path)),'settings':asdict(c.settings),'options':asdict(c.options),'manual':c.manual} for c in self.clips]}
    def save_project(self):
        ...
        try:
            ...
            self.project_path=path;self.status.setText('项目已保存 · 下次打开会重新分析视频以确认画面位置')
        except Exception as error:self.message(error)
    ...
    def load_project(self,path):
        ...
        try:
            ...
            if data.get('format')!='face-privacy-studio' or data.get('version')!=1:raise ValueError('不是受支持的工作台项目。')
            pending=[]
            ...
            if pending:self.analyze_current()
        except Exception as error:self.message(error)
    ...
    def closeEvent(self,event):
        if self.busy:
            self.cancel.set();self.status.setText('正在取消处理，完成后可关闭窗口。');event.ignore();return
        self.player.stop();self.player.setSource(QUrl())
        for c in self.clips:
            if c.analysis:
...
def main():
    ...
    if '--verify' in sys.argv:
        from acceptance import run_verification
        i=sys.argv.index('--verify');run_verification(window,Path(sys.argv[i+1]),Path(sys.argv[i+2]))
        return app.exec()
    def start_session():
